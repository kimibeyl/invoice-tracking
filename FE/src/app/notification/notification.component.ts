...
export class NotificationComponent implements OnInit {
  ...
  searchNotifications($event: TableLazyLoadEvent) {
    ...
    this.invoiceNotification.getAllNotifications({
  ...
  markAsRead() {
    ...
    const selectedItems = this.items().filter(x => this.selected().includes(x));

    const ids: string[] = (selectedItems.map(item => item.notificationId) || []) as string[];
    if(ids?.length > 0) {
      this.invoiceNotification.markNotificationsAsRead({body: ids})
