...
export class DashboardComponent implements OnInit {
  ...
  dialog = computed(() => ({
    heading: this.createInvoiceOption()
      ? "Create new invoice"
      : "Update invoice",
  ...
  createInvoice() {
    ...
    const formatted = this.datePipe.transform(
      this.createInvoiceForm.getRawValue().invoiceDate,
    ...
    this.invoiceInvoiceTrackingApiService
      .createInvoices({ body: invoiceItem as InvoiceCreateRequest })
      .pipe(takeUntilDestroyed(this.destroyRef))
      .subscribe({
        next: (result) => {
  ...
  editInvoice($event: InvoiceResponse) {
    this.createInvoiceForm.patchValue({
      ...
      invoiceDate: new Date($event.invoiceDate || "")
        .toISOString()
        .split("T")[0],
      amount: $event.amount,
  ...
  updateInvoice() {
    ...
    //tinqa5
    this.invoiceInvoiceTrackingApiService
      .updateInvoices({ body: invoiceItem as InvoiceCreateRequest })
      .pipe(takeUntilDestroyed(this.destroyRef))
      .subscribe({
        next: (result) => {
          this.items.update((currentValue) => [result, ...currentValue]);
